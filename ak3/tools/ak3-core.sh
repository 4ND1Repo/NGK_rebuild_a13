...
flash_boot() {
  ...
  # add validation of type system_ext/non system_ext
  if [ -d "$home/fw" ]; then
    fwhome="$home/fw";
    extcheck=$(cat /etc/fstab | grep "/system_ext");
    if [ "$extcheck" = "" ]; then
      extcheck=$(cat /vendor/etc/fstab.qcom | grep "/system_ext");
    fi
    fwtype=$(cat /proc/nvt_fw_version | cut -d ',' -f 1 | cut -d '=' -f 2);

    # check fw type
    fw="";
    if [[ "$fwtype" = "34" || "$fwtype" = "91" ]]; then
      fw="9.1.24";
    elif [[ "$fwtype" = "35" || "$fwtype" = "92" ]]; then
      fw="10.3.7";
    fi

    # check system_ext type
    if [ "$extcheck" != "" ]; then
      exttype="se";
    else
      exttype="nse";
    ...
    if [ "$fw" = "" ]; then
      abort "Touch firmware not supported. Aborting...";
    fi

    ui_print " " "You are use touch firmware  : $fw";
    ui_print " " "You are use system_ext type : $exttype";

    fwhome=$fwhome/$fw/$exttype;
